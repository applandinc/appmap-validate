...
const getCallTag = (event) => {
  ...
  if (
    Reflect.getOwnPropertyDescriptor(event, "http_client_request") !== undefined
  ) {
    return "http-client";
  }
  if (
    Reflect.getOwnPropertyDescriptor(event, "http_server_request") !== undefined
  ) {
    return "http-server";
  }
...
const getReturnTag = (event) => {
  ...
  if (
    Reflect.getOwnPropertyDescriptor(event, "http_client_response") !==
    undefined
  ) {
    return "http-client";
  }
  if (
    Reflect.getOwnPropertyDescriptor(event, "http_server_response") !==
    undefined
  ) {
    return "http-server";
  }
  ...
  for (let index1 = 0; index1 < events.length; index1 += 1) {
    ...
    if (event1.event === "call") {
      ...
      const index2 = events.findIndex(
        (event2) => event2.id === event1.parent_id
      );
      assert(
        index2 !== -1,
        "missing matching call event for #%i -- ie: %o",
        index1,
        event1
      );
      const event2 = events[index2];
